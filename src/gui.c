...
static UINT32 app_color(UINT16 kind, const Gfx *gfx) {
    ...
    if (kind == 5) return gfx_rgb(gfx, 0x2f, 0xa3, 0x9d);  // vpn
...
void gui_render_desktop(Gfx *gfx, int installed, int privacy_strict, int vpn_enabled) {
    ...
    UINT32 privacy_chip = privacy_strict ? gfx_rgb(gfx, 0x2c, 0x68, 0xd6) : gfx_rgb(gfx, 0x6f, 0x6f, 0x6f);
    UINT32 vpn_chip = vpn_enabled ? gfx_rgb(gfx, 0x2f, 0xa3, 0x9d) : gfx_rgb(gfx, 0x7a, 0x4a, 0x4a);
    gfx_fill(gfx, 18, 7, 120, 18, install_chip);
    gfx_fill(gfx, 146, 7, 120, 18, privacy_chip);
    gfx_fill(gfx, 274, 7, 120, 18, vpn_chip);

    for (UINT32 i = 0; i < 7; i++) {
        gfx_fill(gfx, 210 + i * 40, gfx->height - 34, 28, 20, gfx_rgb(gfx, 0x33, 0x4a, 0x64));
    }

    UINT32 shortcuts = installed ? 4 : 5;
