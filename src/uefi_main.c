...
static const uint8_t VPN_EXU[] = {
    'E','X','U','1',0x01,0x00,0x05,0x00,0x1c,0x00,0x00,0x00,
    'N','e','b','u','l','a',' ','T','u','n','n','e','l','|','v','p','n',':','/','/','s','e','c','u','r','e','-','m','o','d','e'
};
...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *st) {
    ...
    PrivacyState privacy = {0};
    privacy_load(st, &privacy);
    ...
    if (!parse_app(VPN_EXU, sizeof(VPN_EXU), &apps[app_count++])) return 5;

    if (!privacy.installed) {
        if (!parse_app(INSTALLER_EXU, sizeof(INSTALLER_EXU), &apps[app_count++])) return 6;
        privacy.installed = 1;
        privacy.privacy_strict = 1;
        privacy.vpn_enabled = 1;
        privacy_persist(st, &privacy);
        st->ConOut->OutputString(st->ConOut, L"Live mode: install + privacy profile enabled. Reboot to hide installer.\r\n");
    }

    gui_render_desktop(&gfx, privacy.installed, privacy.privacy_strict, privacy.vpn_enabled);
